    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        ...
        if self.end_pose_to_action:
            ...
            self.end_pose_head = nn.Sequential(
                nn.Linear(hidden_dim, end_pose_dim),
                nn.ReLU()  # Add ReLU activation for non-linearity
            )
            self.end_pose_mid = nn.Sequential(
                nn.Linear(end_pose_dim, middle_dim),
                nn.ReLU()  # Add ReLU activation for non-linearity
            )
...
class EPDETRVAE_V2(nn.Module):
    """ Drived from DETRVAE, but with end_pose_to_action """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
        ...
        if self.end_pose_to_action:
            ...
            self.end_pose_mid1 = nn.Sequential(
                nn.Linear(end_pose_dim, middle_dim1),
                nn.ReLU()  # Add ReLU activation for non-linearity
            )
            self.end_pose_mid2 = nn.Sequential(
                nn.Linear(middle_dim1, middle_dim2),
                nn.ReLU()  # Add ReLU activation for non-linearity
            )
            self.action_head = nn.Linear(middle_dim2, action_dim)
        else:
            self.action_head = nn.Linear(hidden_dim, action_dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.backbones is not None:
            ...
            hs = self.transformer(transformer_input, None, self.query_embed.weight, self.pos.weight)[0]
        end_pose = self.end_pose_head(hs)
        end_pose_mid1 = self.end_pose_mid1(end_pose)
        end_pose_mid2 = self.end_pose_mid2(end_pose_mid1)
        a_hat = self.action_head(end_pose_mid2)
        # print("hs:", hs.shape)
        # print("end_pose:", end_pose.shape)
        # print("end_pose_mid:", end_pose_mid.shape)
        # print("a_hat:", a_hat.shape)
        ...
        is_pad_hat = self.is_pad_head(hs)
        return a_hat, is_pad_hat, [mu, logvar], probs, binaries, end_pose


class EPDETRVAE_V1(nn.Module):
    """ Drived from DETRVAE, but with end_pose_to_action """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
...
class EPDETRVAE_V4(nn.Module):
    """ V4 Drived from DETRVAE, but with end_pose_to_action """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
...
class EPDETRVAE(nn.Module):
    """ V5 Drived from DETRVAE, but with end_pose_to_action """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
        ...
        if self.end_pose_to_action:
            ...
            self.end_pose_head = nn.Linear(hidden_dim, end_pose_dim)
            self.aux_action_mid =  nn.Sequential(
                nn.Linear(hidden_dim, aux_mid_dim),
                nn.ReLU()  # Add ReLU activation for non-linearity
            )
            ...
            # stack aux_action_mid and end_pose_mid2
            self.action_head = nn.Linear(middle_dim2+aux_mid_dim, action_dim)
        else:
            self.action_head = nn.Linear(hidden_dim, action_dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        end_pose_mid1 = self.end_pose_mid1(end_pose)
        end_pose_mid2 = self.end_pose_mid2(end_pose_mid1)
        aux_mid = self.aux_action_mid(hs)
        aux_concat = torch.cat([end_pose_mid2, aux_mid], axis=2)

        a_hat = self.action_head(aux_concat)

        # print("hs:", hs.shape)
        # print("end_pose:", end_pose.shape)
        # print("end_pose_mid:", end_pose_mid1.shape)
        # print("end_pose_mid2:", end_pose_mid2.shape)
        # print("aux_mid:", aux_mid.shape)
        # print("aux_concat:", aux_concat.shape)
        # print("a_hat:", a_hat.shape)

        is_pad_hat = self.is_pad_head(hs)
        return a_hat, is_pad_hat, [mu, logvar], probs, binaries, end_pose

class EPDETRVAE_EFF_IND(nn.Module):
    """ V6 Drived from DETRVAE, but with end_pose_to_action, End Effector action independent """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
        ...
        """
        super().__init__()
        print("EPDETRVAE V6 End Effector Independent")
        self.num_queries = num_queries
        self.camera_names = camera_names
        self.transformer = transformer
        self.encoder = encoder
        ...
        if self.end_pose_to_action:
            ...
            self.eff_action_head =  nn.Linear(hidden_dim, end_eff_dim)
            self.joints_action_head = nn.Linear(middle_dim2, action_dim - end_eff_dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        end_pose_mid1 = self.end_pose_mid1(end_pose)
        end_pose_mid2 = self.end_pose_mid2(end_pose_mid1)
        eef_action = self.eff_action_head(hs)
        joints_action = self.joints_action_head(end_pose_mid2)
        a_hat = torch.cat([eef_action, joints_action], axis=2)
        # print("hs:", hs.shape)
        # print("end_pose:", end_pose.shape)
        # print("end_pose_mid:", end_pose_mid1.shape)
        # print("end_pose_mid2:", end_pose_mid2.shape)
        # print("eef_action:", eef_action.shape)
        # print("joints_action:", joints_action.shape)
        # print("a_hat:", a_hat.shape)
...
class EPDETRVAE_V3(nn.Module):
    """ V3 Drived from DETRVAE, but with end_pose_to_action """
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
...
def build_epact(args):
    ...
    print("build_epact.eff_independent: ", args.eff_independent)
    if not args.eff_independent:
        model = EPDETRVAE(
            backbones,
            transformer,
            encoder,
